...
@section('title', $quote->exists ? 'Edit About Quote' : 'Add About Quote')

@section('content')
    <div class="p-6 max-w-3xl mx-auto">
        {{-- Breadcrumb/Back Link --}}
        <div class="mb-6">
            <a href="{{ route('about-quote.index') }}"
                class="text-sm text-black hover:text-[#9a9a1e] transition-colors flex items-center space-x-1 font-semibold">
                <i class="fas fa-arrow-left text-xs"></i>
                <span>Back to About Quotes</span>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="bg-primary px-8 py-6">
                <h1 class="text-2xl font-bold text-white tracking-tight flex items-center">
                    <i class="fas {{ $quote->exists ? 'fa-edit' : 'fa-plus-circle' }} mr-3 opacity-80"></i>
                    {{ $quote->exists ? 'Edit About Quote' : 'Add New About Quote' }}
                </h1>
                <p class="text-indigo-100/80 text-sm mt-1">Manage icons, subtitles, and detailed quotes for the About Us
                    section.</p>
            </div>
            ...
            <div class="p-8">
                ...
                <form method="POST"
                    action="{{ $quote->exists ? route('about-quote.update', $quote) : route('about-quote.store') }}"
                    class="space-y-6">
                    @csrf
                    @if ($quote->exists)
                        @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="icon" name="icon" value="{{ old('icon', $quote->icon) }}"
                                    ...
                                    placeholder="e.g. fa-solid fa-mountain-sun">
                            </div>
                            <p class="text-[10px] text-gray-400 font-medium ml-1 uppercase tracking-tighter">
                                Browse icons at <a href="https://fontawesome.com/search" target="_blank"
                                    class="text-primary hover:underline">fontawesome.com</a>
                            </p>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="subtitle" name="subtitle"
                                    value="{{ old('subtitle', $quote->subtitle) }}"
                                    class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all text-gray-800 font-medium"
                    ...
                    {{-- Quote Textarea --}}
                    <div class="space-y-1">
                        <label for="quote" class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Quote
                            Content</label>
                        <textarea id="quote" name="quote" rows="4"
                            ...
                            placeholder="Enter the full quote description..." required>{{ old('quote', $quote->quote) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center space-x-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="status" value="1" class="sr-only peer"
                                {{ old('status', $quote->status) ? 'checked' : '' }}>
                            <div
                        ...
                        <span class="text-sm font-bold text-gray-700 uppercase tracking-wide">Publish to Website</span>
                        <p class="text-xs text-gray-400 font-medium ml-4">(Inactive quotes will be hidden from public view)
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100 mt-8">
                        ...
                        <button type="submit"
                            ...
                            <span>{{ $quote->exists ? 'Update Quote' : 'Save Quote' }}</span>
                        </button>
                    </div>
                </form>
            </div>
