...
@section('title', (isset($galleryContent) ? 'Edit' : 'Add') . ' Gallery Image')

@section('content')
    <div class="p-6 max-w-3xl mx-auto">
        {{-- Breadcrumb/Back Link --}}
        <div class="mb-6">
            <a href="{{ route('gallery-contents.index') }}"
                class="text-sm text-black hover:text-[#9a9a1e] transition-colors flex items-center space-x-1 font-semibold">
                <i class="fas fa-arrow-left text-xs"></i>
                <span>Back to Gallery Contents</span>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="bg-primary px-8 py-6">
                <h1 class="text-2xl font-bold text-white tracking-tight flex items-center">
                    <i class="fas {{ isset($galleryContent) ? 'fa-edit' : 'fa-plus-circle' }} mr-3 opacity-80"></i>
                    {{ isset($galleryContent) ? 'Edit Gallery Image' : 'Add New Gallery Image' }}
                </h1>
                <p class="text-indigo-100/80 text-sm mt-1">Upload an image, set a title, and assign a category for the
                    gallery section.</p>
            </div>
            ...
            <div class="p-8">
                ...
                <form method="POST" enctype="multipart/form-data"
                    action="{{ isset($galleryContent) ? route('gallery-contents.update', $galleryContent) : route('gallery-contents.store') }}"
                    class="space-y-6">
                    @csrf
                    @isset($galleryContent)
                        @method('PUT')
                    @endisset

                    {{-- Image Upload Section --}}
                    <div class="space-y-1">
                        <label for="image" class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Gallery
                            Image *</label>
                        <div class="relative">
                        ...
                        @isset($galleryContent)
                            <div class="mt-4 p-2 bg-gray-50 rounded-xl border border-dashed border-gray-300 inline-block">
                                ...
                                <img src="{{ asset('storage/' . $galleryContent->image) }}" class="h-32 rounded-lg shadow-sm">
                            </div>
                        @endisset
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            <label for="title"
                                class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Image Title *</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                                    <i class="fas fa-heading text-xs"></i>
                                </span>
                                <input type="text" id="title" name="title"
                                    value="{{ old('title', $galleryContent->title ?? '') }}"
                                    ...
                                    required placeholder="e.g. Modern Suite View">
                            </div>
                        </div>
                        ...
                        <div class="space-y-1">
                            <label for="category"
                                class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Category *</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                                    <i class="fas fa-filter text-xs"></i>
                                </span>
                                <select name="category" id="category"
                                    class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all text-gray-800 font-medium appearance-none">
                                    @foreach (['exterior', 'interiors', 'culture', 'dining'] as $cat)
                                        <option value="{{ $cat }}"
                                            {{ old('category', $galleryContent->category ?? '') == $cat ? 'selected' : '' }}>
                                            {{ ucfirst($cat) }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="sort_order" name="sort_order"
                                value="{{ old('sort_order', $galleryContent->sort_order ?? 0) }}"
                                class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all text-gray-800 font-medium">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer"
                                {{ old('is_active', $galleryContent->is_active ?? 1) == 1 ? 'checked' : '' }}>
                            <div
                        ...
                        <span class="text-sm font-bold text-gray-700 uppercase tracking-wide">Visible in Gallery</span>
                    </div>
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100 mt-8">
                        ...
                        <button type="submit"
                            ...
                            <span>{{ isset($galleryContent) ? 'Update Image' : 'Save Image' }}</span>
                        </button>
                    </div>
                </form>
            </div>
