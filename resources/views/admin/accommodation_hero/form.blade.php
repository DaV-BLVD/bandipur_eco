{{-- resources/views/admin/accommodation_hero/form.blade.php --}}
@extends('admin.layouts.app')

@section('title', $hero->exists ? 'Edit Accommodation Hero' : 'Add Accommodation Hero')

@section('content')
<div class="p-6 max-w-3xl mx-auto">
    {{-- Breadcrumb/Back Link --}}
    <div class="mb-6">
        <a href="{{ route('accommodation-hero.index') }}" class="text-sm text-black hover:text-[#9a9a1e] transition-colors flex items-center space-x-1 font-semibold">
            <i class="fas fa-arrow-left text-xs"></i>
            <span>Back to Accommodation Hero</span>
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
        ...
        <div class="bg-primary px-8 py-6">
            <h1 class="text-2xl font-bold text-white tracking-tight flex items-center">
                ...
                {{ $hero->exists ? 'Edit Accommodation Hero' : 'Add New Accommodation Hero' }}
            </h1>
            <p class="text-indigo-100/80 text-sm mt-1">Upload a hero image and manage its display for the accommodation page.</p>
        </div>
        ...
        <div class="p-8">
            ...
            <form method="POST" 
                  action="{{ $hero->exists ? route('accommodation-hero.update', $hero) : route('accommodation-hero.store') }}" 
                ...
                <div class="space-y-1">
                    <label for="image" class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Hero Image</label>
                    <div class="relative">
                ...
                {{-- Alt Text Section --}}
                <div class="space-y-1">
                    <label for="alt_text" class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Alt Text</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                            <i class="fas fa-quote-left text-xs"></i>
                        </span>
                        <input type="text" id="alt_text" name="alt_text" 
                            value="{{ old('alt_text', $hero->alt_text) }}"
                ...
                <div class="flex items-center space-x-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <label class="relative inline-flex items-center cursor-pointer">
                        {{-- Note: name="status" used here to match your original database field --}}
                        <input type="checkbox" name="status" value="1" class="sr-only peer" {{ old('status', $hero->status) ? 'checked' : '' }}>
                    ...
                    <p class="text-xs text-gray-400 font-medium ml-4">(Toggle to enable/disable this image on the website)</p>
                </div>

                {{-- Action Buttons --}}
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100 mt-8">
                    <a href="{{ route('accommodation-hero.index') }}"
