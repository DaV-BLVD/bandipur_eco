{{-- resources/views/admin/who-we-are-photos/index.blade.php --}}
@extends('admin.layouts.app')

@section('title', 'Who We Are Photos')
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                    <i class="fas fa-users mr-2 text-primary"></i> Who We Are Photos
                </h1>
                <p class="text-sm text-gray-500 mt-1">Manage the visual imagery for the "Who We Are" section.</p>
            </div>

            {{-- <a href="{{ route('who-we-are-photos.create') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <thead class="bg-primary text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-widest">Primary Image</th>
                            <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-widest">Content Info</th>
                            <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-widest">Secondary Image</th>
                        ...
                        @forelse($photos as $photo)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div
                                        class="h-16 w-24 overflow-hidden rounded-lg border border-gray-200 shadow-sm bg-gray-50">
                                        <img src="{{ asset('storage/' . $photo->image_primary) }}" alt="Primary"
                                ...
                                {{-- Title & Subtitle --}}
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-gray-900">{{ $photo->title ?? 'No Title' }}</div>
                                    <div class="text-xs text-gray-500 italic">{{ $photo->subtitle ?? '-' }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div
                                        class="h-16 w-24 overflow-hidden rounded-lg border border-gray-200 shadow-sm bg-gray-50">
                                        <img src="{{ asset('storage/' . $photo->image_secondary) }}" alt="Secondary"
                                            class="h-full w-full object-cover">
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <div class="flex justify-center items-center space-x-2">
                                        {{-- Edit --}}
                                        <a href="{{ route('who-we-are-photos.edit', $photo) }}"
                                        ...
                                        {{-- Delete --}}
                                        {{-- <form method="POST" action="{{ route('who-we-are-photos.destroy', $photo) }}"
                                            onsubmit="return confirm('Are you sure you want to delete this set of photos?')"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 font-medium">No photos found for the "Who We Are" section.
                                        </p>
                                        <a href="{{ route('who-we-are-photos.create') }}"
                                            class="mt-2 text-primary hover:underline text-sm font-semibold">Upload your
                                            first set of images</a>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
