...
@section('title', 'Contact Hero Form')

@section('content')
<div class="p-6 max-w-3xl mx-auto">
    {{-- Breadcrumb/Back Link --}}
    <div class="mb-6">
        <a href="{{ route('contact-hero.index') }}" class="text-sm text-black hover:text-[#9a9a1e] transition-colors flex items-center space-x-1 font-semibold">
            <i class="fas fa-arrow-left text-xs"></i>
            <span>Back to Contact Hero</span>
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
        ...
        <div class="bg-primary px-8 py-6">
            <h1 class="text-2xl font-bold text-white tracking-tight flex items-center">
                ...
                {{ $hero->exists ? 'Edit Contact Hero' : 'Add New Contact Hero' }}
            </h1>
            <p class="text-indigo-100/80 text-sm mt-1">Upload a hero image and set its visibility for the contact page.</p>
        </div>
        ...
        <div class="p-8">
            ...
            <form method="POST" 
                  action="{{ $hero->exists ? route('contact-hero.update', $hero) : route('contact-hero.store') }}" 
                ...
                <div class="space-y-1">
                    ...
                    @if($hero->image)
                        <div class="mt-4 p-2 bg-gray-50 rounded-xl border border-dashed border-gray-300 inline-block">
                            ...
                            <img src="{{ asset('storage/'.$hero->image) }}" class="h-32 rounded-lg shadow-sm">
                        </div>
                    @endif
                </div>
                ...
                <div class="flex items-center space-x-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $hero->is_active) ? 'checked' : '' }}>
