...
@section('title', $item->exists ? 'Edit Grid Item' : 'Add Grid Item')

@section('content')
    <div class="p-6 max-w-4xl mx-auto">
        {{-- Breadcrumb/Back Link --}}
        <div class="mb-6">
            <a href="{{ route('about-images-grid.index') }}"
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="bg-primary px-8 py-6 text-white">
                <h1 class="text-2xl font-bold tracking-tight flex items-center">
                    <i class="fas {{ $item->exists ? 'fa-edit' : 'fa-plus-circle' }} mr-3 opacity-80"></i>
                    {{ $item->exists ? 'Edit Grid Item' : 'Add New Grid Item' }}
                </h1>
                <p class="text-indigo-100/80 text-sm mt-1">Add or update an image, its title, and its position within the
                    gallery grid.</p>
            </div>
            ...
            <div class="p-8">
                ...
                <form method="POST"
                    action="{{ $item->exists ? route('about-images-grid.update', $item) : route('about-images-grid.store') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            <label for="title"
                                class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Item Title</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                                    <i class="fas fa-tag text-xs"></i>
                                </span>
                                <input type="text" name="title" id="title" placeholder="e.g. Master Bedroom"
                    ...
                    <div class="space-y-1">
                        <label for="description"
                            class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Short Description</label>
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Briefly describe this grid item...">{{ old('description', $item->description) }}</textarea>
                    </div>

                    {{-- Image Upload Section --}}
                    <div class="space-y-1">
                        <label for="image" class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Grid
                            Image</label>
                        <div class="flex items-start space-x-6">
                            <div class="flex-1">
                                ...
                                <p class="text-[10px] text-gray-400 mt-2 uppercase font-bold tracking-widest">Recommended:
                                    Square or 4:3 Aspect Ratio</p>
                            </div>

                            @if ($item->image)
                                <div class="p-2 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                                    <p class="text-[9px] uppercase font-bold text-gray-400 mb-1">Current Image</p>
                    ...
                    <div class="relative py-4">
                        ...
                        <div class="relative flex justify-start text-sm font-bold uppercase tracking-widest">
                            <span class="bg-white pr-4 text-primary">Visibility</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        ...
                        <span class="text-sm font-bold text-gray-700 uppercase tracking-wide">Publish to Grid</span>
                        <p class="text-xs text-gray-400 font-medium ml-4">(Active items will be visible to the public)</p>
                    </div>
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100 mt-8">
                        ...
                        <button type="submit"
                            ...
                            <span>{{ $item->exists ? 'Update Grid Item' : 'Create Grid Item' }}</span>
                        </button>
                    </div>
                </form>
            </div>
