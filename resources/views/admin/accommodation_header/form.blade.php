{{-- resources/views/admin/accommodation_header/form.blade.php --}}
@extends('admin.layouts.app')

@section('title', $header->exists ? 'Edit Accommodation Header' : 'Add Accommodation Header')

@section('content')
    <div class="p-6 max-w-3xl mx-auto">
        {{-- Breadcrumb/Back Link --}}
        <div class="mb-6">
            <a href="{{ route('accommodation-header.index') }}"
                class="text-sm text-black hover:text-[#9a9a1e] transition-colors flex items-center space-x-1 font-semibold">
                <i class="fas fa-arrow-left text-xs"></i>
                <span>Back to Accommodation Headers</span>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="bg-primary px-8 py-6">
                <h1 class="text-2xl font-bold text-white tracking-tight flex items-center">
                    ...
                    {{ $header->exists ? 'Edit Accommodation Header' : 'Add New Accommodation Header' }}
                </h1>
                <p class="text-indigo-100/80 text-sm mt-1">Define the badge, main title, and description for the accommodation
                    section header.</p>
            </div>
            ...
            <div class="p-8">
                ...
                <form method="POST"
                    action="{{ $header->exists ? route('accommodation-header.update', $header) : route('accommodation-header.store') }}"
                    class="space-y-6">
                    @csrf
                    @if ($header->exists)
                        @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="badge_text" name="badge_text"
                                    ...
                                    placeholder="e.g. LUXURY ROOMS" required>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1">
                            <label for="title"
                                class="block text-sm font-bold text-gray-700 uppercase tracking-wider">Header Title</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            placeholder="Enter a brief description for the accommodation page..." required>{{ old('description', $header->description) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center space-x-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <label class="relative inline-flex items-center cursor-pointer">
                            {{-- Note: name matches your table field 'status' --}}
