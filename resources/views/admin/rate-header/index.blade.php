        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                    <i class="fas fa-percentage mr-2 text-primary"></i> Rate Headers
                </h1>
                <p class="text-sm text-gray-500 mt-1">Manage room types, discounts, and service configurations for the rates
                    page.</p>
            </div>

            {{-- <a href="{{ route('rate-header.create') }}"
                class="flex items-center space-x-2 bg-primary text-white px-5 py-2.5 rounded-xl font-semibold transition-all hover:opacity-90 shadow-lg shadow-indigo-100 focus:ring-2 focus:ring-primary focus:ring-opacity-50">
                <i class="fas fa-plus text-xs"></i>
                <span>Add Rate Header</span>
            </a> --}}
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <thead class="bg-primary text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-widest">Rate Stats</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($headers as $header)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                {{-- Title --}}
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-gray-900">{{ $header->title }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-y-1 gap-x-4">
                                        <div class="flex items-center text-xs text-gray-600">
                                            <i class="fas fa-bed mr-1.5 text-primary opacity-70"></i>
                                            <span class="font-semibold">{{ $header->room_types }}</span>&nbsp;Rooms
                                        </div>
                                        <div class="flex items-center text-xs text-gray-600 border-l border-gray-200 pl-4">
                                            <i class="fas fa-tag mr-1.5 text-primary opacity-70"></i>
                                            <span class="font-semibold">{{ $header->off_season_discount }}%</span>&nbsp;Off
                                        </div>
                                        <div class="flex items-center text-xs text-gray-600 border-l border-gray-200 pl-4">
                                            <i class="fas fa-clock mr-1.5 text-primary opacity-70"></i>
                                            <span class="font-semibold">{{ $header->service_hours }}h</span>&nbsp;Service
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="{{ route('rate-header.edit', $header) }}"
                                            class="p-2 text-secondary hover:bg-green-400 hover:text-white rounded-lg transition-all"
                                            title="Edit Rate">
                                            <i class="fas fa-edit"></i>
                                        </a>

                                        {{-- <form method="POST" action="{{ route('rate-header.destroy', $header) }}"
                                              onsubmit="return confirm('Are you sure you want to delete this rate header?');"
                                              class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 font-medium">No rate headers found.</p>
                                        <a href="{{ route('rate-header.create') }}"
